...
def number_of_nodes():
	result = crsr.execute('SELECT COUNT(*) FROM nodes')
	return result.fetchone()[0]

def number_of_ways():
	result = crsr.execute('SELECT COUNT(*) FROM ways')
	return result.fetchone()[0]

def number_of_unique_users():
	result = crsr.execute('SELECT COUNT(DISTINCT(e.uid)) \
            FROM (SELECT uid FROM nodes UNION ALL SELECT uid FROM ways) e')
	return result.fetchone()[0]
    
def top_contributing_users():
	users = []
	for row in crsr.execute('SELECT e.user, COUNT(*) as num \
...
def number_of_users_contributing_once():
	result = crsr.execute('SELECT COUNT(*) \
            FROM \
...
def common_amenities():
	for row in crsr.execute('SELECT value, COUNT(*) as num \
            FROM nodes_tags \
            WHERE key="amenity" \
            GROUP BY value \
            ORDER BY num DESC \
...
def biggest_religion():
	for row in crsr.execute('SELECT nodes_tags.value, COUNT(*) as num \
            FROM nodes_tags \
